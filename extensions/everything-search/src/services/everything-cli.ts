...
export async function searchFilesWithCLI(searchText: string, preferences: Preferences): Promise<FileInfo[]> {
  ...
  try {
    ...
    if (useRegex) {
      // Extract command-line options (like -case, -i, -w, etc.) from searchText
      const optionRegex = /^((?:-\w+\s+)*)(.*)/;
      const match = searchText.match(optionRegex);
      options = match ? match[1].trim() : "";
      const actualSearchText = match ? match[2].trim() : searchText;
      searchQuery = `-r "${actualSearchText}"`;
    }

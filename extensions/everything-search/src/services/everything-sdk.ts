...
switch (platform) {
  case "win32":
    switch (arch) {
      case "x64":
        try {
          everythingSDK = require(join(environment.assetsPath, "/native/everything-search-sdk-64.node"));
        } catch (e) {
      ...
      case "arm64":
        try {
          everythingSDK = require(join(environment.assetsPath, "/native/everything-search-sdk-arm.node"));
        } catch (e) {
          loadError = e as Error;
        }
        break;
